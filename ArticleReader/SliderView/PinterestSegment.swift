...
@objc public protocol PinterestSegmentDelegate:NSObjectProtocol{
    func pinterestSegmentDidTap(at: Int);
}
...
@IBDesignable public class PinterestSegment: UIControl {
    ...
    public var delegate : PinterestSegmentDelegate?
...
//        view.contentOffset = CGPoint.zero
        ...
        for (i, label) in titleLabels.enumerated() {
            if x >= label.frame.minX && x <= label.frame.maxX {
                ...
                if let handler = delegate{
                    handler.pinterestSegmentDidTap(at: i);
                }
                break
            }
        }
    ...
    private func reloadData() {
        ...
        let toToSize: (String) -> CGFloat = { text in
            return (text as NSString).boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: 0.0), options: .usesLineFragmentOrigin, attributes: nil, context: nil).width
        }
